  ...
  <div class='publish-detail-container'>
    ...
    <div class="carousel-section">
      <van-swipe loop :autoplay='3000' indicator-color="white" class="custom-swipe">
        ...
        <van-swipe-item v-if="!publish.imgArr || publish.imgArr.length === 0">
          <div class="empty-image"><van-icon name="photo-o" size="40" color="#c8c9cc" /><p>该需求暂无图片</p></div>
        </van-swipe-item>
      </van-swipe>
    </div>
    ...
    <div class="info-card">
      <div class="card-header">
        ...
        <div class="price-box">
          <span class="price-label">{{ publish.valuation ? '最终价格' : '系统预估价' }}</span>
          <span class="price-value">{{ publish.valuation ? '¥' + publish.valuation?.toFixed(2) : (publish.estimatedPrice ? '¥' + publish.estimatedPrice?.toFixed(2) : '等待计算') }}</span>
        </div>
      </div>
      <van-divider dashed :style="{ borderColor: '#ebedf0', margin: '10px 0' }" />
      <div class="card-body"><p class="content-desc">{{ publish.content }}</p></div>
    </div>
    ...
    <div class="bottom-action-bar">
      <van-button v-if='user.userType === 2 && publish.status === 0' round block type='info' color="linear-gradient(to right, #ff6034, #ee0a24)" @click='onValuation(publish.id)'>
        我要抢单 / 报价
      </van-button>
    ...
    <van-popup v-model='valuationShow' round class="valuation-popup">
      <div class="valuation-form">
        <h3>请输入您的报价</h3>
        <p class="subtitle">参考系统预估价: <strong style="color:#ee0a24;">¥{{ publish.estimatedPrice }}</strong></p>
        <van-field v-model='valuationForm.valuation' type='number' label='接单价格' placeholder='请输入金额' :rules="[{ required: true }]" class="price-input">
          <template #right-icon><span style="color: #323233; font-weight: bold;">元</span></template>
        </van-field>
        <div class="submit-wrap"><van-button @click='valuationSubmit' type='info' color="linear-gradient(to right, #ff6034, #ee0a24)" round block>确认提交报价</van-button></div>
      </div>
    </van-popup>
  </div>
</template>
...
<script>
import { findPublishApi, valuationPublishApi } from '@/api/publish'
import { mapState } from 'vuex'

export default {
  data() { return { id: null, valuationShow: false, publish: { imgArr: [] }, valuationForm: { id: 0, valuation: 0 } } },
  methods: {
    ...
    onValuation(id) {
      ...
      // 默认将预估价填入输入框，司机可以修改，体验极佳！
      this.valuationForm.valuation = this.publish.estimatedPrice;
      this.valuationShow = true
    },
    async findPublish() {
      const { data } = await findPublishApi(this.id)
      data.imgArr = data.imgs ? data.imgs.split(',').filter(e => !!e) : []
      if (data.cover) data.imgArr.unshift(data.cover)
      this.publish = data
    }
  },
  created() { this.id = this.$route.params.id; this.findPublish() },
...
.custom-swipe { background-color: #fff; } .empty-image { height: 260px; display: flex; flex-direction: column; justify-content: center; align-items: center; background-color: #f7f8fa; color: #969799; font-size: 14px; }
...
.valuation-popup { width: 90%; max-width: 340px; } .valuation-form { padding: 24px 20px; text-align: center; h3 { margin: 0; font-size: 18px; color: #323233; } .subtitle { margin: 6px 0 20px; font-size: 12px; color: #969799; } .price-input { background-color: #f7f8fa; border-radius: 8px; margin-bottom: 24px; } }
</style>